...
template <typename L,typename T,class tag_storage=generic_trie_bimap<T,small_state_t,state_t>>
struct SWA {
  ...
  bool has_state(state_t const& s) const {
      return adj.find(s) != end(adj);
  }

  bool has_acc(state_t const& s) const {
      return acc.find(s) != end(acc);
  }
  ...
  void remove_states(vector<state_t> const& tokill) {
    ...
    for (auto const& it : tokill) {
        auto el = adj.find(it);
        if (el != end(adj))
          adj.erase(el); //kill state + edges
    }
    for (auto const& it : tokill) {
        auto el = acc.find(it);
        if (el != end(acc))
          acc.erase(el); //kill acc. mark
    }
    ...
    for (auto& it : adj) {
      for (auto& jt : it.second) {
        vector<state_t> res;
        set_difference(begin(jt.second), end(jt.second),
                        begin(tokill), end(tokill), back_inserter(res));
        jt.second = res;
      }
    }
  }
